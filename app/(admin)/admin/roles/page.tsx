        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-[--color-border] bg-[--color-muted]/50 text-left text-xs uppercase tracking-wider text-[--color-muted-fg]">
              <tr>
                <th className="sticky left-0 bg-[--color-muted]/50 px-4 py-3">Module</th>
                {roles.map((r) => (
            ...
            <tbody>
              {ALL_MODULES.map((mod) => (
                <tr key={mod} className="border-b border-[--color-border] last:border-0">
                  <td className="sticky left-0 bg-[--color-card] px-4 py-3 font-medium">{mod}</td>
                  {roles.map((r) => {
                    const grant = ROLE_MATRIX[r.name]?.[mod];
                    const label =
                      grant === "*" ? "All" : Array.isArray(grant) ? grant.join(", ") : "—";
                    return (
                      <td key={r.id} className="px-4 py-3 text-center text-xs text-[--color-muted-fg]">
                        {label}
                      </td>
                    );
                  })}
